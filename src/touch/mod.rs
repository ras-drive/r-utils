use crate::touch::arg_parser::Args;
...
pub fn run(matches: ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    if Path::exists(Path::new(args.filename)) {
        if matches.is_present("date") {
            ...
            set_file_atime(args.filename, FileTime::from_unix_time(date.timestamp(), 0))
                .expect("TODO: panic message");
            set_file_mtime(args.filename, FileTime::from_unix_time(date.timestamp(), 0))
                .expect("TODO: panic message");
        }
